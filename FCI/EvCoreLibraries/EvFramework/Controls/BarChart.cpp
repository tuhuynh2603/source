 ...
 ********************************************************************/
///Udupa; 12May'2013; Added clustered bar graph display with custom gap
...
{
	m_bars.AddBar(udValue, szLabel, color, -1);
...
void CBarChart::AddBars(CString szLabel, int nCount, int* nValues, COLORREF* colors, BOOL bRepaint)
{
	for(int i=0; i<nCount; i++)
		m_bars.AddBar(nValues[i], (i==(nCount/2)) ? szLabel : "", colors[i], i==(nCount-1) ? BAR_DEFAULT_GAP*3/2 : 2);
...
void CBarChart::DrawTitle(CDC* pDC)
{
	...
	pDC->DrawText(m_szTitle,
		CRect(rcTitle.left+1 ,rcTitle.top+1, rcTitle.right, rcTitle.bottom)
		, DT_CENTER | DT_SINGLELINE | DT_VCENTER);

	pDC->SetTextColor(RGB(150,200,200));
...
void CBarChart::OnSize(UINT nType, int cx, int cy) 
{
	...
	m_rctClient = CRect(0,0,cx,cy);
	m_bars.m_sBarPicSize.cy = cy;
...
void CBarChart::SetBarGap(int nGap)
{
	m_bars.m_nDefaultBarGap = nGap;
}

void CBarChart::SetBarWidth(int nWidth)
{
	m_bars.m_nBarWidth = nWidth;
}
...
CBarChart::CChartBar::CChartBar()
{
	...
	m_nDefaultBarGap = BAR_DEFAULT_GAP;
...
{
	int nBarGap = m_chartData.GetGap(nIndex);
	m_chartData.RemoveAt(nIndex);
	m_sBarPicSize.cx = 0;
	for(int i=0; i<m_chartData.GetCount(); i++)
		m_sBarPicSize.cx += m_nBarWidth + m_chartData.GetGap(i);
}
...
void CBarChart::CChartBar::CreateLabelFont()
{
	...
	strcpy_s(m_lf.lfFaceName,"Segoe UI");
	m_lf.lfHeight=18;
	m_lf.lfWidth=0;
	m_lf.lfEscapement = 0;
...
void CBarChart::CChartBar::AddBar(double udValue,CString szLabel, COLORREF color, int barGap)
{
	m_chartData.Add(szLabel, udValue, color, barGap<0 ? BAR_DEFAULT_GAP : barGap);
	
	int nCount = m_chartData.GetCount();
	m_sBarPicSize.cx = 0;
	for(int i=0; i<nCount; i++)
		m_sBarPicSize.cx += m_nBarWidth + m_chartData.GetGap(i);
	if(nCount > 0)
		m_sBarPicSize.cx -= m_chartData.GetGap(nCount-1);
}
...
//	}
	...
	for (int i = 0; i<m_chartData.GetCount();i++)
	{
		...
		x = 0;
		for(int j=0; j<i; j++)
			x += m_nBarWidth + m_chartData.GetGap(j);
		//x = i*(m_nBarWidth + m_chartData.GetGap(i));
		...
		if (m_bShowBarText || m_bShowLabel)
		{
			...
			//int nHalfGap = m_chartData.GetGap(i)*2/3;
			int nHalfGap = BAR_DEFAULT_GAP*2/3;
			if (m_bShowBarText)
			{
				...
				if (m_nBarTextType== 0 )
				{
					...
					pDC->DrawText(str, CRect(x, y - BAR_TXT_PERCENT_HEIGHT, x+ m_nBarWidth + m_chartData.GetGap(i)-1, y),
						DT_LEFT | DT_SINGLELINE | DT_VCENTER);
				}
				else if (m_nBarTextType==1)		// Draw value at the top of the bar
				{
					str.Format("%0.f", m_chartData.GetValue(i));
					pDC->DrawText(str, CRect(x - nHalfGap, y - BAR_TXT_PERCENT_HEIGHT, x+ m_nBarWidth + nHalfGap, y),//+ m_nBarGap-1
						DT_CENTER | DT_SINGLELINE | DT_VCENTER);
				}
			}
			...
			if (m_bShowLabel)
			{
				...
				pDC->DrawText(str, CRect(x - nHalfGap, y + (int)nHeight , x+ m_nBarWidth + nHalfGap, y + (int)nHeight + BAR_TXT_LABEL_HEIGHT),
					DT_CENTER | DT_SINGLELINE | DT_VCENTER);
...
return;
	...
	else
	{
 	...
 	m_dIndex = (int) ( (double)(point.x - m_rMargin*m_dAlpha) / 
						 (double)((m_nBarWidth*m_dAlpha) + (m_nDefaultBarGap*m_dAlpha)) );
...
void CBarChart::CChartData::Add (CString& szLabel, double udValue, COLORREF color, int barGap)
{
	data *pData;
...
int CBarChart::CChartData::GetGap(int nIndex)
{
	if (nIndex<0 || nIndex>=m_arr.GetSize())
	{
		return -1;
	...
	return ((data*)m_arr.GetAt(nIndex))->m_nBarGap;
}
