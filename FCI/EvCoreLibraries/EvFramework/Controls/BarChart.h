...
#define CHART_TITLE_HEIGHT		40	// 20; This reserves enough room for the title at the bottom of the chart. If you don't need a title, set it to zero
...
#define BAR_DEFAULT_WIDTH		44	// 24; Default width of each bar, increase it to make bars fat
#define BAR_DEFAULT_GAP			30	// 4; Distance between bars
#define BAR_REFLECT_AMOUNT		32	// Decrease to make WHITE shines more

#define BAR_TXT_PERCENT_HEIGHT	30	// 10; Room for a Value/Percent at the top of each bar
#define BAR_TXT_LABEL_HEIGHT		40	// 10;Room for the label at the buttom of each bar
...
protected:
	...
	public:
		...
		int GetCount();
		void Add (CString& szLabel, double udValue, COLORREF color, int barGap);
		
		double GetMaxValue();
		double GetValue(int nIndex);
		int GetGap(int nIndex);
	...
	public:
		...
		void RemoveAll();
		void RemoveAt(int nIndex);
		void AddBar(double udValue,CString szLabel, COLORREF color, int barGap);
...
public:
	...
	void AddBars(CString szLabel, int nCount, int* unValues, COLORREF* colors, BOOL bRepaint=FALSE);

	void SetBKColor(COLORREF colorBK);
	
	void Refresh();
	void SetBarWidth(int nWidth);
	void SetBarGap(int nGap);
