...
CEvHistoryImagePane::CEvHistoryImagePane()
{
	m_nSourceImage = 0;
	m_nDepth = 15;
...
void CEvHistoryImagePane::OnClose() 
{
	CloseChildren();
...
void CEvHistoryImagePane::Clean(int nIndex)
{
...
BEGIN_MESSAGE_MAP(CEvHistoryImagePane, CDockablePane)
	ON_WM_CREATE()
	ON_WM_SIZE()
	ON_WM_CLOSE()
END_MESSAGE_MAP()

int CEvHistoryImagePane::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
void CEvHistoryImagePane::AddImageView()
{
...
void CEvHistoryImagePane::SetHistoryDepth(int nDepth)
{
	int nCount = (int) m_arrImageWnds.GetCount();
...
void CEvHistoryImagePane::SetTitles()
{
	CEvSdiChildWnd* pWnd;
	for(int i=0; i<m_nDepth; i++) {
...
void CEvHistoryImagePane::PushImage(CImageBuffer* pBuffer)
{
	m_nLastIndex++;
...
void CEvHistoryImagePane::AdjustLayout()
{
	CRect rect;
	GetWindowRect(&rect);
	...
	if(m_nDepth==15) {
		cx /= 5;
		cy /= 3;
		int arrcx1[15] = { 0, cx,  cx<<1, cx*3, cx*4,  0, cx,  cx<<1, cx*3, cx*4 ,0, cx,  cx<<1, cx*3, cx*4 };
		int arrcy1[15] = { 0,  0,      0,    0,    0, cy, cy,     cy,   cy,   cy,cy<<1, cy<<1,     cy<<1,   cy<<1,   cy<<1 };
...
void CEvHistoryImagePane::OnSize(UINT nType, int cx, int cy)
{
...
void CEvHistoryImagePane::AdjustHorzScroll(CListBox& wndListBox)
{
	//CClientDC dc(this);
...
void CEvHistoryImagePane::UpdateFonts()
{
...
CEvHistoryImageView::CEvHistoryImageView()
{
}
...
BEGIN_MESSAGE_MAP(CEvHistoryImageView, CEvTabView)
	ON_WM_CONTEXTMENU()
...
void CEvHistoryImageView::OnContextMenu(CWnd* pWnd, CPoint point)
{
	CMenu menu;
...
void CEvHistoryImageView::OnViewOutput()
{
