...
BOOL CDIBSectionLite::SetColorTable(int nNumColors, RGBQUAD *pColors)
{
//	m_pRGBptr = pColors;
	BYTE * byteValue = (BYTE*)pColors;

	for(int i = 0;i<nNumColors;i++)
	{
		m_DIBinfo.arColors[i].rgbRed = *byteValue++;
		m_DIBinfo.arColors[i].rgbGreen = *byteValue++;
		m_DIBinfo.arColors[i].rgbBlue = *byteValue++;
		m_DIBinfo.arColors[i].rgbReserved = 0;
		*byteValue++;
	}
...
BOOL CDIBSectionLite::CreatePalette()
{
    ...
    RGBQUAD *pRGB = new RGBQUAD[abs((int) m_iColorTableSize)];
    if (!pRGB)
...
UINT CDIBSectionLite::GetColorTableEntries(HDC hdc, HBITMAP hBitmap)
{
    ...
    UINT nCount;
    RGBQUAD* pRGB = new RGBQUAD[abs(m_iColorTableSize)];
    if (pRGB)
    {
...
RGBQUAD * CDIBSectionLite::GetColorTablePtr()
{
	return m_pRGBptr;
