...
CMd5::CMd5(const char* text, int nLength)
{
  init();
  update(text, nLength);
  finalize();
...
///Udupa; March'2014
void CMd5::hexdigest(CString& str)
{
  if (!finalized) {
	str = "";
    return;
...
///Udupa; March'2014
CString md5(CString& strInput)
{
    CMd5 md5 = CMd5(strInput.GetBuffer(0), strInput.GetLength());
	strInput.ReleaseBuffer();
 
	CString strMd5;
	md5.hexdigest(strMd5);
    return strMd5;
}

///Udupa; March'2014
CString EncryptDecrypt(CString& strInput)
{
    char key = (char) 0xda;
    ...
    for (int i = 0; i < nLen; i++)
        strOutput.SetAt(i, strInput[i] ^ key);
